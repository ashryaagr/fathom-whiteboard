...
<img src="resources/icon.png" alt="clawdSlate" width="128" height="128" />

<img src="resources/hero.png" alt="Paste anything. Get a whiteboard." width="560" />

<!-- TODO: hero GIF showing paste → diagram in motion -->

clawdSlate is a standalone Mac app for paste-driven whiteboard brainstorming with Claude. Drop in a paragraph, an abstract, an image, or a PDF — watch the agent draw an explanatory diagram on a live Excalidraw canvas you can edit alongside.

For now: macOS + Claude Code subscription. Windows, Linux, Codex, and Gemini support coming soon.

[![Release](https://img.shields.io/github/v/release/ashryaagr/clawdslate?label=release&color=f59e0b)](https://github.com/ashryaagr/clawdslate/releases/latest)
[![Platform](https://img.shields.io/badge/macOS-arm64-lightgrey)](#install)
[![License](https://img.shields.io/github/license/ashryaagr/clawdslate)](./LICENSE)
[![Stars — clawdSlate](https://img.shields.io/github/stars/ashryaagr/clawdslate?style=social)](https://github.com/ashryaagr/clawdslate)
...
[Documentation](https://ashryaagr.github.io/clawdslate/) · [Install guide](./docs/INSTALL.md) · [How it works](#how-it-works) · [Principles](./docs/PRINCIPLES.md) · [Methodology](./docs/methodology/index.md) · [Build from source](#build-from-source) · [All releases](https://github.com/ashryaagr/clawdslate/releases)
...
- **Grounded in what you paste.** No retrieval, no embeddings, no general-purpose web lookup. clawdSlate hands the agent your pasted content and a single MCP server (`excalidraw-mcp`) and asks it to draw what's actually there. If something's wrong, you can see why by reading the same content the agent did.
...
clawdSlate's primary install path is the terminal:

```bash
curl -fsSL https://raw.githubusercontent.com/ashryaagr/clawdslate/main/install.sh | bash
```

The script downloads the app, extracts to `/Applications`, clears the `com.apple.quarantine` xattr (so Gatekeeper doesn't ask for approval on first launch), ad-hoc re-signs, and drops a `clawdslate` launcher at `~/.local/bin/clawdslate` so you can `clawdslate` from any terminal. Same script handles updates — re-run it or type `clawdslate update`.
...
Download the Mac DMG: [`clawdSlate-arm64.dmg`](https://github.com/ashryaagr/clawdslate/releases/latest/download/clawdSlate-arm64.dmg). The [install guide](./docs/INSTALL.md#option-b--dmg) walks you through the one-time Gatekeeper approval that DMG users see on first launch. Both paths converge on the same `clawdSlate.app`.

Intel Macs aren't shipped in v0.1.x — build from source if you need x64 today (see below).

### Embedding inside another app?

clawdSlate's whiteboard component is also published on npm as `fathom-whiteboard`:
...
clawdSlate runs the agent through the [Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk-typescript), which wraps the Claude Code CLI. You'll need:
...
- [ ] **Claude Code CLI installed** (`claude` on your `$PATH`).
   ...
   claude --version
   ```
- [ ] **Claude Code signed in.** clawdSlate uses your existing Claude subscription through the CLI — no API keys.
   ```bash
   claude /login
   ```

If `claude` isn't on `$PATH` when clawdSlate launches, the app surfaces the exact command to run. Re-launch and continue.
...
```
   ...
   │
   ├─► clawdSlate hands the content + a system prompt to Claude (Agent SDK)
   │     system prompt = the coleam excalidraw SKILL + a clawdSlate suffix
...
- **Zero telemetry.** clawdSlate doesn't phone home about what you paste or what diagrams you draw. The only outbound traffic is the Claude CLI talking to Anthropic on your existing subscription.
...
- **Uses your existing Claude subscription.** Whatever your team already pays for Claude works for clawdSlate; no new procurement.

Most whiteboard tools want a vendor relationship. clawdSlate is a Mac app you compiled yourself.

## Methodology

[docs/methodology/index.md](./docs/methodology/index.md) is the engineering write-up of how clawdSlate's pipeline works: how the SKILL prompt is delivered, what the `allowedTools` lock-down does, why the hosted MCP endpoint is the default, and what the cost profile looks like in practice (about $0.95/paper for generation, $0.10–$0.30 per refinement turn).
...
npm run dist:mac          # produce release/clawdSlate-arm64.{dmg,zip}
```
...
├── app/
│   ├── main.ts            Electron main process (clawdSlate-only)
...
clawdSlate is grounded in a small set of cognitive-science results. Citations are listed honestly — where the project's earlier notes overstated an effect size, this section uses the conservative form.

- **Diagrams aid reasoning.** Larkin & Simon (1987) showed that diagrammatic representations make information *explicit through spatial indexing* that sentential representations leave implicit, reducing search and inference cost. (*Why a Diagram is (Sometimes) Worth Ten Thousand Words*, Cognitive Science 11(1).) clawdSlate's whiteboard is a reasoning surface, not decoration: relations are placed in 2D space the reader can scan parafoveally instead of being chased through prose.

- **Element interactivity drives cognitive load.** Sweller (2010) reformulates intrinsic cognitive load as the count of *interacting* elements a learner must hold simultaneously, not the count of elements. (*Element Interactivity and Intrinsic, Extraneous, and Germane Cognitive Load*, Educational Psychology Review 22(2).) Implication for clawdSlate's layout: the diagrams' shape should match the subject's actual coupling structure — boxes that interact get drawn near each other, boxes that don't get separated by whitespace.

- **Self-explanation.** Chi et al. (1989) showed that learners who articulate the connection between an example and the underlying principle outperform those who don't. (*Self-Explanations: How Students Study and Use Examples in Learning to Solve Problems*, Cognitive Science 13(2).) Replicated in Chi et al. 1994. clawdSlate operationalises this: the user types the prompt — the act of articulating what they want explained is itself the work that produces understanding. There is no "auto-explain" path.

- **Graphic organisers help, modestly.** Luiten, Ames & Ackerson's (1980) meta-analysis of 135 studies found a *small but reliable* facilitative effect of advance organisers on learning and retention. (*A Meta-analysis of the Effects of Advance Organizers on Learning and Retention*, American Educational Research Journal 17(2).) clawdSlate's whiteboard is a graphic structural overview of pasted content — it helps, but clawdSlate doesn't sell it as a giant learning multiplier.

## See also

- [**Fathom**](https://github.com/ashryaagr/Fathom) — research-paper reader with the same whiteboard tab inside it. Use Fathom when you want to read and zoom into a paper; use clawdSlate when you want to paste anything (slides, code, screenshots) and get a diagram. The same `fathom-whiteboard` npm package powers both.
...
- [**writing-resources/awesome-scientific-writing**](https://github.com/writing-resources/awesome-scientific-writing) — academic writing tools beyond LaTeX (Markdown editors, Zotero, Pandoc, Quarto, Jupyter Book). Pairs naturally with clawdSlate, which sits at the input end of the same workflow.
...
For bug reports, the DevTools console log (Cmd+Option+I in the running app) is more useful than a screenshot — every subsystem emits `[clawdSlate …]` or `[fathom-whiteboard …]` lines so we can trace a failure end-to-end.
