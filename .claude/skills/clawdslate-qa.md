...
clawdSlate is a visual app: paste anything, watch the agent draw on a
...
1. **DevTools console + main-process stdout.** clawdSlate emits
   `[clawdSlate …]` (main + preload) and `[fathom-whiteboard …]`
   (pipeline) lines for every IPC + agent turn. After every
   harness command:

   ```bash
   # Main-process logs land in stdout/stderr of the launched clawdSlate;
...
2. **Persisted state on disk.**
   ```bash
   ls "$HOME/Library/Application Support/clawdSlate/sessions/last/"
...
```bash
screencapture -x /tmp/clawdslate-shots/welcome.png
```

Then read that file path and grade it yourself. Good expectation
phrasings:

- ✓ "A light-grey window with the clawdSlate wordmark in handwritten
...
```bash
screencapture -v -R 0,0,1280,880 -T 0 /tmp/clawdslate-stream.mov
# ... paste content, hit return, watch the canvas stream ...
# Ctrl+C to stop

# Sample frames with ffmpeg, then read
ffmpeg -i /tmp/clawdslate-stream.mov -vf fps=10 /tmp/frames/f-%03d.png
```
...
still broken" reports — applies equally to clawdSlate. Multiple agent claims
...
### Why this matters in clawdSlate

clawdSlate's `npm run app` script runs `npm run app:build` first
...
TEST_USER_DATA="/tmp/clawdslate-test-$(date +%s)"
rm -rf "$TEST_USER_DATA"
# (if clawdSlate supports --user-data-dir override directly via Electron flag,
# pass it; otherwise launch a separately-built test build that points
# app.setPath('userData', TEST_USER_DATA) at startup)
open -a clawdSlate --args --user-data-dir="$TEST_USER_DATA"
sleep 3

# 1. Welcome / paste-prompt
screencapture -x /tmp/clawdslate-shots/01-welcome.png
# Expect: "clawdSlate" wordmark in handwriting at the top, empty canvas,
...
osascript -e 'tell application "System Events" to key code 36'  # Return
sleep 30
screencapture -x /tmp/clawdslate-shots/04-refined.png
...
open -a clawdSlate --args --user-data-dir="$TEST_USER_DATA"
sleep 4
screencapture -x /tmp/clawdslate-shots/05-restored.png
...
The "scene survives restart" bug class is one of clawdSlate's most
load-bearing invariants (CLAUDE.md §1: "Persist by default. Once
the user has paid the API cost to generate a whiteboard,
regenerating it because clawdSlate forgot to save is a design
...
3. Compare. Don't just say "looks right" — state the structural
   facts you see. "clawdSlate wordmark visible top-left, paste-prompt
   bottom, canvas centred."
4. Flag any of these categories of failure:
   - Missing element (no canvas where there should be)
   - Wrong text (placeholder says "Ask clawdSlate" when spec says "Paste anything")
   - Wrong font (handwritten where it should be sans, or vice
     versa — see `clawdslate-communication.md` for the policy)
...
clawdSlate needs Claude Code installed + signed in. Users will hit the
...
```bash
# Launch clawdSlate with /usr/bin:/bin:/sbin only, hiding every .local/bin
# or Homebrew path where claude normally sits. This is what clawdSlate
# will see for a user who never installed Claude Code.
env PATH=/usr/bin:/bin:/sbin open -gj -a clawdSlate
```
...
defaulting to a path inside `app.asar`. clawdSlate's pipeline picks a
...
```bash
# Open a fresh release/clawdSlate-arm64.app and confirm a generate run
...
**Rule**: every prerequisite clawdSlate depends on gets one of these
